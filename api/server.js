...
//
// these are just express() middleware, just like the ones that we write.
//
...
// own quick attempt to do what morgan() does...
...
//    LOCKOUT()
//----------------------------------------------------------------------------//
// lockout() is another middleware demonstration... the method for adding
...
// this unless you want to hork your entire api.
//
...
server.get('/', (req, res) => {
  ...
  res.send(`
    ...
    <p>Welcome to the Lambda Hubs API, ${name}!</p>
    `);
});
...
// are just JavaScript objects. And, when control is passed to the "next" middleware,
...
// an error is passed using next(), the error object or content is passed
...
// "next()'ing" will allow you to go back to it. There is no previous()!!!
//
...
// or data, and with a 500 result code. If the error parameter has a "status"
// property, it will respond with that result code instead of 500.
//
...
// ExpressJS, it's only the number of parameters that matters.
//

server.use((error, req, res, next) => {
  console.log("I'm an error handler!");

  const message = error.whitelisted ? error.message : "Oops! Something didn't work"; // error.whitelisted and error.message are available to inspect and act upon
